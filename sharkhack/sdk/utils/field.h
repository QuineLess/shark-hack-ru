...
public:
	...
	{
		unsigned field_t = *reinterpret_cast<uint16_t*>(data);
		unsigned type = field_t & TagTypeMask;
		...
		if (field_t & 0x80) {
			field_t = ((field_t & 0x7f) | ((field_t & 0xff00) >> 1)) >> TagTypeBits;
			bytesRead = 2;
		}
		else {
			field_t = (field_t & 0xff) >> TagTypeBits;
			bytesRead = 1;
		}
		...
		return field(field_t, type, value, full);
	}
