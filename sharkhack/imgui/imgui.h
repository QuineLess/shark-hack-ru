...
inline ImFont* roboto_bold_elements;
inline ImFont* roboto_regular_checkbox;
inline ImFont* roboto_medium;
...
namespace ImGui
{
	...
	IMGUI_API void			Anim(float * pAnim, bool IsTrue, float min, float max, float strong, float strong2);

	// Widgets: Main
    // - Most widgets return true when the value has been changed or when pressed/selected
    // - You may also use one of the many IsItemXXX functions (e.g. IsItemActive, IsItemHovered, etc.) to query widget state.
    IMGUI_API bool          Button(const char* label, const ImVec2& size = ImVec2(0,0));    // button
	IMGUI_API bool			ButtonSubtabs(const char * label, const ImVec2 & size_arg, bool selected = false);
	IMGUI_API bool			ButtonTabs(const char * label, const ImVec2 & size_arg, bool selected = false);
    IMGUI_API bool          ComboButton(const char* label, const ImVec2& size = ImVec2(0, 0), bool selected = false);
	IMGUI_API bool			CheckboxButton(const char * label, const ImVec2 & size_arg, bool * v);
	IMGUI_API bool		 CheckboxButtonBar(const char * label, const ImVec2 & size_arg, bool * v);
	IMGUI_API bool          ListButton(const char* label, const ImVec2& size = ImVec2(0, 0), bool selected = false);
	...
	IMGUI_API void			ResetAnimations();
	...
	IMGUI_API ImVec4		ColConvertToFloat(float r, float g, float b, float a = 255.f);
...
enum ImGuiWindowFlags_
{
	...
	ImGuiWindowFlags_NoChildShadow			= 1 << 29,  // Shadow for Child
...
enum ImGuiCol_
{
    ...
    ImGuiCol_CheckboxFrame,
    ImGuiCol_CheckboxFrameActive,
    ImGuiCol_SliderGrab,
...
struct ImGuiStyle
{
    ...
    ImVec2      ComboButtonSize;               // Padding within a framed rectangle (used by most widgets).
    float       PopupRounding;              // Radius of popup window corners rounding. (Note that tooltip windows use WindowRounding)
    float       PopupBorderSize;            // Thickness of border around popup/tooltip windows. Generally set to 0.0f or 1.0f. (Other values are not well tested and more CPU/GPU costly).
    ImVec2      FramePadding;               // Padding within a framed rectangle (used by most widgets).
    ImVec2      ShadowSize;               // Padding within a framed rectangle (used by most widgets).
